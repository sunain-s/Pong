import pygame, sys, random, mode_select, win_screen
...
class Ball(Block):
	...
	def reset_ball(self):
		...
		self.speed_y = randint(1, 7) * random.choice((-1,1))
...
class GameManager:
	...
	def reset_ball(self):
		if self.ball_group.sprite.rect.right >= screen_width:
			...
			if self.opponent_score == mode_select.multi_player.end_score:
				win_screen.window_m_player2()
			self.ball_group.sprite.reset_ball()
		if self.ball_group.sprite.rect.left <= 0:
			self.player_score += 1
			if self.player_score == mode_select.multi_player.end_score:
				win_screen.window_m_player1()
			self.ball_group.sprite.reset_ball()
...
def run():
	...
	while running:
		...
		for event in pygame.event.get():
			# if quit, returns to mode select
			...
			if event.type == pygame.KEYDOWN:
				...
				if event.key == pygame.K_ESCAPE:
					running = False
				# player inputs up and down
				if event.key == pygame.K_UP:
					player.movement -= player.speed
				if event.key == pygame.K_DOWN:
					player.movement += player.speed
				# opponent inputs w and s
